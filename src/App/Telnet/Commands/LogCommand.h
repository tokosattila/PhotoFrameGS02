  ...
  class LogCommand_ : public Command_ {
    public:
      ...
      bool Execute(const char *tArguments, WiFiClient &tClient) override {
        ...
        if (strcmp(tArguments, "status") == 0) return HandleStatus(tClient);
        else if (strcmp(tArguments, "info") == 0) return HandleInfo(tClient);
        else if (strcmp(tArguments, "flush") == 0) return HandleFlush(tClient);
        tClient.print(F("Unknown log command: "));
        tClient.println(tArguments);
        return false;
      }
      const char *Help() const override {
        return "log                              - logging status & management";
      }
    private:
      bool HandleStatus(WiFiClient &tClient) {
        ...
        if (tNow >= 1000000000L) {
          ...
          char tDatePath[64] = "";
          snprintf(tDatePath, sizeof(tDatePath), "logs/%04d/%02d/%02d/", tTm.tm_year + 1900, tTm.tm_mon + 1, tTm.tm_mday);
          tClient.print(F("  Log Directory:  "));
          tClient.println(tDatePath);
          char tDateStr[16] = "";
          snprintf(tDateStr, sizeof(tDateStr), "%04d%02d%02d", tTm.tm_year + 1900, tTm.tm_mon + 1, tTm.tm_mday);
